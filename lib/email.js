...
/**
 * Resolve the technical From address. Business + Enterprise detailers with
 * a verified custom_email_domain send through their own domain via Resend
 * Domains. Free + Pro use the platform sending domain.
 */
function getFromAddress(detailer) {
  const plan = (detailer?.plan || 'free').toLowerCase();
  const eligible = plan === 'business' || plan === 'enterprise';
  if (
    eligible &&
    detailer?.custom_email_domain &&
    detailer?.custom_email_verified_at
  ) {
    return `noreply@${detailer.custom_email_domain}`;
  }
  return (FROM_EMAIL.match(/<([^>]+)>/) || [null, FROM_EMAIL])[1];
}

/**
 * Plan-aware branded FROM header for customer-facing email.
 *
 *   free       → "Shiny Jets — {company} <noreply@mail.shinyjets.com>"
 *   pro        → "{company} <noreply@mail.shinyjets.com>"
 *   business   → "{company} <noreply@mail.shinyjets.com>"
 *   enterprise → "{company} <noreply@{custom_email_domain}>" if verified,
 *                else "{company} <noreply@mail.shinyjets.com>"
 *
 * For platform → detailer mail (signup, password reset, comp invite delivery,
 * billing) keep the un-detailered FROM_EMAIL — those aren't customer-facing.
 */
function getBrandedFrom(detailer) {
  ...
  const displayName = branding.isFree
    ? `Shiny Jets — ${companyName}`
    : companyName;
  return `${displayName} <${getFromAddress(detailer)}>`;
}

/**
 * Customer replies should land in the detailer's inbox, not Brett's.
 */
function getReplyToFor(detailer) {
  return detailer?.email || DEFAULT_REPLY_TO;
}
...
function withFooter(html, recipientEmail) {
  if (!html) return html;
  // Skip if footer already injected (legacy class name preserved so the guard
  // still matches older deployed mail)
  if (html.includes('__sj-footer__')) return html;

  const unsub = `${UNSUBSCRIBE_URL}?email=${encodeURIComponent(recipientEmail || '')}`;
  // CAN-SPAM-compliant footer that does not name the SaaS platform — customers
  // relate to the detailer, not to Vector/Shiny Jets. The physical address
  // remains on the sending company (required by US law for commercial mail).
  ...
  <p style="margin:0;">To stop receiving these emails, <a href="${unsub}" style="color:#999;text-decoration:underline;">unsubscribe here</a>.</p>
</div>`;
...
/**
 * Customer-facing send — plan-aware From + detailer-routed Reply-To in one
 * shot. Prefer this over sendEmail() at any site that mails the detailer's
 * customers (photo requests, quotes, invoices, change orders, follow-ups,
 * reminders, blasts). Internal/platform mail (signup, comp invite delivery)
 * should keep using sendEmail() directly.
 */
export async function sendCustomerEmail({ detailer, to, subject, html, text, preheader }) {
  return sendEmail({
    to,
    subject,
    html,
    ...
    replyTo: getReplyToFor(detailer),
    preheader,
  });
}
 ...
 */
export async function sendEmail({ to, subject, html, text, replyTo, from, preheader, attachments }) {
  try {
    ...
    console.log(`Sending email: from=${fromAddr} to=${to} subject=${subject}${attachments?.length ? ` attachments=${attachments.length}` : ''}`);
 ...
 */
export async function sendQuoteSentEmail({ quote, detailer, language, customSubject, customBody, attachments }) {
