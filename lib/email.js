 ...
 */
export async function sendFollowupViewedNotAcceptedEmail({ quote, detailer }) {
  if (!quote.client_email) {
    return { success: false, error: 'No client email' };
  }

  const template = followupViewedNotAcceptedTemplate({ quote, detailer });
 ...
 */
export async function sendFollowupExpiryWarningEmail({ quote, detailer }) {
  if (!quote.client_email) {
    return { success: false, error: 'No client email' };
  }

  const template = followupExpiryWarningTemplate({ quote, detailer });
