...
<div style="max-width:560px;margin:0 auto;padding:32px 20px;">
  ...
  const sent = [];
  const channels = [];
  if (process.env.RESEND_API_KEY && crewWithEmail.length > 0) {
    ...
    if (sent.length > 0) channels.push('email');
  }
  ...
  // Nothing actually delivered → don't stamp, so it can be retried.
  if (sent.length === 0 && !channels.includes('in_app')) {
    return { ok: false, status: 500, error: 'Briefing could not be delivered', source };
  }
