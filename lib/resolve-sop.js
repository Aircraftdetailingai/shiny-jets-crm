...
export async function loadSopContext(supabase, { detailer_id, aircraft_id }) {
  ...
  // sop_file_path / sop_file_name are surfaced too so callers (notably
  // the briefing email) can sign the file path with the right expiry.
  const { data: services } = await supabase
    .from('services')
    .select('id, name, sop_url, sop_summary, sop_file_path, sop_file_name, created_at')
  ...
  if (aircraft_id) {
    const { data: overrides } = await supabase
      ...
      .select('id, service_id, sop_url, sop_summary, sop_file_path, sop_file_name')
      .eq('detailer_id', detailer_id)
    ...
    for (const o of overrides || []) {
      overridesByService.set(o.service_id, {
        ...
        file_path: o.sop_file_path || null,
        file_name: o.sop_file_name || null,
      });
    }
  }
...
function makeContext({ servicesById, servicesByName, overridesByService }) {
  return {
    ...
    resolve(item) {
      ...
      // A service-level SOP exists if it has either a URL or an uploaded
      // file path. Both fields surfaced — callers prefer file_path when
      // present (signs it for short-lived or briefing-email access).
      const hasDefault = !!(service?.sop_url || service?.sop_file_path);
      const defaultSop = hasDefault
        ? {
            url: service.sop_url || null,
            summary: service.sop_summary || null,
            file_path: service.sop_file_path || null,
            file_name: service.sop_file_name || null,
          }
        : null;
