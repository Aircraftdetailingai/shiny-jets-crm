...
export async function exchangeCodeForTokens(code, redirectUri) {
  const res = await fetch(GOOGLE_TOKEN_URL, {
    ...
    body: new URLSearchParams({
      ...
      redirect_uri: redirectUri || env.GOOGLE_CALENDAR_REDIRECT_URI,
...
export async function refreshAccessToken(refreshToken) {
  ...
  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
    const code = err.error || 'refresh_failed';
    const detail = err.error_description || 'Failed to refresh token';
...
export async function getValidAccessToken(detailerId) {
  ...
  if (!conn.refresh_token) {
    ...
    return { accessToken: null, connection: conn, refreshError: 'missing_refresh_token' };
  }
  ...
  try {
    ...
    await supabase
      ...
      .update({
        ...
        // Successful refresh means the connection is healthy again.
    ...
    return { accessToken: tokens.access_token, connection: { ...conn, access_token: tokens.access_token, needs_reconnect: false } };
  } catch (err) {
    const msg = err?.message || String(err);
    console.error('Failed to refresh Google Calendar token:', msg);
    return { accessToken: null, connection: conn, refreshError: msg };
  }
}
