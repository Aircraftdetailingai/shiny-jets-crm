 ...
 */
function emailWrapper(content, { headerBg = BRAND_COLOR, headerText = '', companyName = '', unsubscribeEmail = '', accentColor = '', logoUrl = '' } = {}) {
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #f0f0ee;">
  <div style="background: linear-gradient(135deg, ${headerBg} 0%, #0a1520 100%); padding: 36px 30px; border-radius: 12px 12px 0 0; text-align: center;">
    ...
    <h1 style="color: ${accent}; margin: 0; font-size: 18px; font-weight: 400; letter-spacing: 0.3px;">${headerText}</h1>
  </div>
...
${detailerEmail ? detailerEmail : ''}
  ...
  return {
    subject: subjectLine,
    html: emailWrapper(content, { headerBg: detailer?.theme_accent || BRAND_COLOR, headerText: `Your Quote from ${companyName}`, companyName, unsubscribeEmail: detailer?.email, accentColor: detailer?.theme_primary, logoUrl: detailer?.theme_logo_url }),
    text,
  };
}
...
View your receipt: ${APP_URL}/q/${share_link}
  ...
  return {
    ...
    html: emailWrapper(content, { headerBg: detailer?.theme_accent || SUCCESS_COLOR, headerText: 'Payment Confirmed', companyName, unsubscribeEmail: detailer?.email, accentColor: detailer?.theme_primary, logoUrl: detailer?.theme_logo_url }),
    text,
  };
}
...
${detailer?.email ? `Contact: ${detailer.email}` : ''}${detailer?.phone ? ` | ${detailer.phone}` : ''}
  ...
  return {
    ...
    html: emailWrapper(content, { headerBg: detailer?.theme_accent || (isNewQuote ? BRAND_COLOR : WARNING_COLOR), headerText, companyName, unsubscribeEmail: detailer?.email, accentColor: detailer?.theme_primary, logoUrl: detailer?.theme_logo_url }),
    text,
  };
}
...
${detailer?.email ? `Contact: ${detailer.email}` : ''}${detailer?.phone ? ` | ${detailer.phone}` : ''}
  ...
  return {
    ...
    html: emailWrapper(content, { headerBg: detailer?.theme_accent || INFO_COLOR, headerText: 'Service Reminder', companyName, unsubscribeEmail: detailer?.email, accentColor: detailer?.theme_primary, logoUrl: detailer?.theme_logo_url }),
    text,
  };
}
...
${detailer?.email ? `Contact: ${detailer.email}` : ''}${detailer?.phone ? ` | ${detailer.phone}` : ''}
  ...
  return {
    ...
    html: emailWrapper(content, { headerBg: detailer?.theme_accent || SUCCESS_COLOR, headerText: 'Service Complete!', companyName, unsubscribeEmail: detailer?.email, accentColor: detailer?.theme_primary, logoUrl: detailer?.theme_logo_url }),
    text,
  };
}
...
Questions? Reply to this email or contact ${companyName} directly.
  ...
  return {
    ...
    html: emailWrapper(content, { headerBg: detailer?.theme_accent || WARNING_COLOR, headerText: 'Your Quote is Waiting', companyName, unsubscribeEmail: detailer?.email, accentColor: detailer?.theme_primary, logoUrl: detailer?.theme_logo_url }),
    text,
  };
}
...
Your feedback helps us improve!
  ...
  return {
    ...
    html: emailWrapper(content, { headerBg: detailer?.theme_accent || BRAND_COLOR, headerText: 'Rate Your Experience', companyName, unsubscribeEmail: detailer?.email, accentColor: detailer?.theme_primary, logoUrl: detailer?.theme_logo_url }),
    text,
  };
}
...
export function quoteExpiringTemplate({ quote, detailer }) {
  ...
  return {
    ...
    html: emailWrapper(content, { headerBg: detailer?.theme_accent || WARNING_COLOR, headerText: 'Quote Expiring Soon', companyName, accentColor: detailer?.theme_primary, logoUrl: detailer?.theme_logo_url }),
    text,
  };
}
