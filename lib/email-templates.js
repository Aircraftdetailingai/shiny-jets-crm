...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #f3f4f6;">
  <div style="background: linear-gradient(135deg, ${headerBg} 0%, #0f2744 100%); padding: 36px 30px; border-radius: 12px 12px 0 0; text-align: center;">
    <div style="margin-bottom: 10px;">
      ${brandName}
    </div>
    <h1 style="color: rgba(255,255,255,0.9); margin: 0; font-size: 18px; font-weight: 400; letter-spacing: 0.3px;">${headerText}</h1>
  </div>
  ...
  <div style="text-align: center; padding: 16px 20px; color: #b0b8c4; font-size: 11px;">
    <p style="margin: 0;">Powered by <a href="https://vectorav.ai" style="color: #b0b8c4; text-decoration: none;">Vector</a></p>
...
export function quoteSentTemplate({ quote, detailer, language }) {
  const { aircraft_model, aircraft_type, total_price, share_link, client_name, services, line_items, valid_until, notes, discount_percent, addon_fees, addon_total, airport, minimum_fee_applied } = quote;
  ...
  const companyName = detailer?.company || detailer?.name || 'Your Detailing Pro';
  ...
  if (lineItems.length > 0) {
    serviceRowsHtml = lineItems.map(item => `
        <tr>
          <td style="padding: 8px 0; color: #374151; font-size: 15px;">${item.description}</td>
          <td style="padding: 8px 0; text-align: right; color: #374151; font-size: 15px;">${formatCurrency(item.amount, currency)}</td>
        </tr>`).join('');
    ...
    serviceRowsHtml = servicesList.map(s => `
        <tr>
  ...
  const discountRowHtml = discountPct > 0 ? `
        <tr>
          <td style="padding: 6px 0; color: #059669; font-size: 14px;">${L.discountLabel} (${discountPct}%)</td>
          <td style="padding: 6px 0; text-align: right; color: #059669; font-size: 14px;">-${discountPct}%</td>
        </tr>` : '';
  ...
  const addonRowsHtml = addons.map(a => `
        <tr>
          <td style="padding: 6px 0; color: #374151; font-size: 15px;">${a.name}</td>
          <td style="padding: 6px 0; text-align: right; color: #374151; font-size: 15px;">${formatCurrency(a.amount, currency)}</td>
        </tr>`).join('');
  ...
  // Detailer contact block
  const contactParts = [detailerName, detailerPhone, detailerEmail].filter(Boolean);
  const contactHtml = contactParts.length > 0 ? `
    <div style="margin-top: 8px; padding-top: 16px; border-top: 1px solid #e5e7eb;">
      <p style="margin: 0; font-size: 14px; color: #374151; font-weight: 600;">${detailerName || companyName}</p>
      <p style="margin: 2px 0 0 0; font-size: 14px; color: #6b7280;">${companyName !== detailerName ? companyName : ''}</p>
      ${detailerPhone ? `<p style="margin: 2px 0 0 0; font-size: 14px;"><a href="tel:${detailerPhone}" style="color: #1e3a5f; text-decoration: none;">${detailerPhone}</a></p>` : ''}
      ${detailerEmail ? `<p style="margin: 2px 0 0 0; font-size: 14px;"><a href="mailto:${detailerEmail}" style="color: #1e3a5f; text-decoration: none;">${detailerEmail}</a></p>` : ''}
    </div>` : '';

  const content = `
    <div style="display:none;font-size:1px;color:#f3f4f6;line-height:1px;max-height:0px;max-width:0px;opacity:0;overflow:hidden;">
      ${L.previewText} ${aircraftDisplay} - ${amount}
    </div>
    ...
    <div style="background: linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%); border: 1px solid #e2e8f0; border-radius: 12px; padding: 24px; margin: 0 0 24px 0;">
      ...
      <table style="width: 100%; border-collapse: collapse;">
        ...
        ${validDate ? `
        <tr>
          <td style="padding: 10px 0; color: #64748b; font-size: 14px;">${L.validUntilLabel}</td>
          <td style="padding: 10px 0; text-align: right; font-size: 14px; color: #1e293b;">${validDate}</td>
        </tr>` : ''}
        <tr>
          <td style="padding: 16px 0 8px 0; border-top: 2px solid #cbd5e1; color: #0f172a; font-weight: 700; font-size: 18px;">${L.totalLabel}</td>
          <td style="padding: 16px 0 8px 0; border-top: 2px solid #cbd5e1; text-align: right; font-weight: 800; font-size: 28px; color: #1e3a5f;">${amount}</td>
        </tr>
      </table>
    </div>
    ...
    <div style="text-align: center; margin: 32px 0 16px 0;">
      <a href="${APP_URL}/q/${share_link}" style="display: inline-block; background: linear-gradient(135deg, #ea580c 0%, #d97706 100%); color: #ffffff; padding: 18px 48px; text-decoration: none; border-radius: 10px; font-weight: 700; font-size: 18px; letter-spacing: 0.3px; mso-padding-alt: 0; text-align: center; box-shadow: 0 4px 14px rgba(217, 119, 6, 0.4);">
    ...
    <div style="margin-top: 24px;">
      ...
      <p style="font-size: 14px; color: #64748b; margin: 0 0 16px 0;">
        ${L.questionsLine} <a href="tel:${detailerPhone}" style="color: #1e3a5f; font-weight: 600; text-decoration: none;">${detailerPhone}</a>.
      </p>` : ''}
...
${discountPct > 0 ? `${L.discountLabel}: -${discountPct}%` : ''}
...
export function invoiceTemplate({ invoice }) {
  ...
  return {
    subject: `✈️ Invoice ${invoice.invoice_number} from ${invoice.detailer_company || invoice.detailer_name || 'your detailer'}`,
...
export function followUpReminderTemplate({ clientName, aircraft, quoteUrl, expiresIn, detailerName, detailerPhone }) {
  ...
  const company = detailerName || 'Your detailer';

  const phoneHtml = detailerPhone
    ? `<p style="font-size: 14px; color: #6b7280;">Or call us at <a href="tel:${detailerPhone}" style="color: #1e3a5f; font-weight: 600;">${detailerPhone}</a></p>`
    : '';
  ...
  const content = `
    ...
    <div style="text-align: center; margin: 30px 0;">
      <a href="${quoteUrl}" style="display: inline-block; background: #1e3a5f; color: #ffffff; padding: 14px 36px; text-decoration: none; border-radius: 6px; font-weight: 600; font-size: 16px;">
        View Your Quote
      </a>
    </div>
...
export function expiryDiscountTemplate({ clientName, aircraft, quoteUrl, discountPercent, originalPrice, discountedPrice, detailerName, detailerPhone, currency }) {
  ...
  const phoneHtml = detailerPhone
    ? `<p style="font-size: 14px; color: #6b7280; text-align: center;">Or call us at <a href="tel:${detailerPhone}" style="color: #1e3a5f; font-weight: 600;">${detailerPhone}</a></p>`
    : '';
  ...
  const content = `
    ...
    <p style="font-size: 15px; color: #374151; margin-bottom: 20px;">
      Hi ${name},<br><br>
      Your quote for <strong>${aircraftDisplay}</strong> expires in <strong>2 days</strong>. Book now and save <strong style="color: #059669;">${discountPercent}%</strong>!
    </p>

    <div style="background: #ecfdf5; border: 1px solid #a7f3d0; border-radius: 8px; padding: 20px; margin-bottom: 20px; text-align: center;">
      <p style="margin: 0; font-size: 13px; color: #065f46; text-transform: uppercase; letter-spacing: 1px;">Limited Time Offer</p>
      <p style="margin: 8px 0 0 0; font-size: 16px; color: #6b7280; text-decoration: line-through;">${original}</p>
      <p style="margin: 4px 0 0 0; font-size: 28px; font-weight: 700; color: #059669;">${discounted}</p>
      <p style="margin: 8px 0 0 0; font-size: 14px; color: #065f46; font-weight: 600;">Save ${discountPercent}%</p>
    </div>

    <div style="text-align: center; margin: 30px 0;">
      <a href="${quoteUrl}" style="display: inline-block; background: #059669; color: #ffffff; padding: 14px 36px; text-decoration: none; border-radius: 6px; font-weight: 600; font-size: 16px;">
        Claim Your Discount
      </a>
    </div>
