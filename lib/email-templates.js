/**
 * Email Templates for Shiny Jets CRM
...
const GOLD_COLOR = '#007CB1';
const SUCCESS_COLOR = '#007CB1';
...
/**
 * Base email wrapper with detailer branding header and subtle Shiny Jets footer
 */
function emailWrapper(content, { headerBg = BRAND_COLOR, headerText = '', companyName = '', unsubscribeEmail = '', accentColor = '', logoUrl = '', fontHeading = '', fontBody = '' } = {}) {
  ...
  // Unsubscribe link falls back to an empty span when the caller didn't provide
  // a detailer mailbox — never expose a platform-branded email address to a
  // customer who only knows the detailer.
  ...
  const brandName = logoUrl
    ...
    : companyName
      ...
      : `<span style="color: #ffffff; font-size: 30px; font-weight: 800; letter-spacing: -0.5px;">&nbsp;</span>`;
...
<body style="font-family: ${bodyFontStack}; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #f0f0ee;">
  ...
  <div style="text-align: center; padding: 16px 20px; color: #b0b8c4; font-size: 11px;">
    ${companyName ? `<p style="margin: 0;">Sent by ${companyName}</p>` : ''}
    ${unsubscribeHtml ? `<p style="margin: 6px 0 0 0;">${unsubscribeHtml}</p>` : ''}
  </div>
</body>
</html>
  `.trim();
...
export function quoteSentTemplate({ quote, detailer, language }) {
  ...
  const i18n = {
    en: {
      ...
      noteLabel: 'Note from', ctaButton: 'Approve &amp; Pay',
    ...
    es: {
      ...
      noteLabel: 'Nota de', ctaButton: 'Aprobar y Pagar',
    ...
    fr: {
      ...
      noteLabel: 'Note de', ctaButton: 'Approuver et Payer',
    ...
    pt: {
      ...
      noteLabel: 'Nota de', ctaButton: 'Aprovar e Pagar',
    ...
    de: {
      ...
      noteLabel: 'Hinweis von', ctaButton: 'Genehmigen &amp; Bezahlen',
    ...
    it: {
      ...
      noteLabel: 'Nota da', ctaButton: 'Approva e Paga',
    ...
    nl: {
      ...
      noteLabel: 'Opmerking van', ctaButton: 'Goedkeuren en Betalen',
    ...
    ja: {
      ...
      noteLabel: 'からのメモ', ctaButton: '承認して支払う',
    ...
    zh: {
      ...
      noteLabel: '来自的备注', ctaButton: '批准并支付',
      urgency: '立即预订以确保您的名额',
  ...
  const subjectLine = firstName
    ? `${firstName}${L.subject} ${companyName} ${L.subjectSuffix}`
    : `${companyName} - ${L.altSubject} ${aircraftDisplay}`;
  ...
  const content = `
    ...
    <div style="text-align: center; margin: 32px 0 16px 0;">
      <a href="${APP_URL}/q/${share_link}" style="display: inline-block; background: linear-gradient(135deg, #007CB1 0%, #a88b3a 100%); color: #0D1B2A; padding: 18px 48px; text-decoration: none; border-radius: 10px; font-weight: 700; font-size: 18px; letter-spacing: 0.3px; mso-padding-alt: 0; text-align: center; box-shadow: 0 4px 14px rgba(201, 168, 76, 0.4);">
...
export function quoteViewedTemplate({ quote, detailer, viewedAt }) {
  ...
  const content = `
    ...
    <div style="background: #f8f6f0; border: 1px solid #007CB1; border-radius: 8px; padding: 16px; margin: 20px 0; text-align: center;">
    ...
    <div style="text-align: center; margin: 30px 0;">
      <a href="${APP_URL}/dashboard" style="display: inline-block; background: #007CB1; color: #0D1B2A; padding: 12px 30px; text-decoration: none; border-radius: 6px; font-weight: 600;">
        View in Dashboard
      </a>
    </div>
  `;
...
export function paymentReceivedTemplate({ quote, detailer, feeBreakdown }) {
  ...
  const content = `
    ...
    <div style="background: #f8f6f0; border: 2px solid #007CB1; border-radius: 8px; padding: 20px; margin: 20px 0; text-align: center;">
      <p style="margin: 0 0 8px 0; color: #8B7332; font-size: 14px;">Payment Received</p>
      <p style="margin: 0; color: #007CB1; font-size: 32px; font-weight: 700;">${amount}</p>
    </div>
...
export function paymentConfirmedTemplate({ quote, detailer, feeBreakdown }) {
  ...
  const content = `
    ...
    <div style="background: #f8f6f0; border: 2px solid #007CB1; border-radius: 8px; padding: 20px; margin: 20px 0; text-align: center;">
      ...
      <p style="margin: 0; color: #007CB1; font-size: 28px; font-weight: 700;">${amount}</p>
    ...
    <div style="text-align: center; margin: 30px 0;">
      <a href="${APP_URL}/q/${share_link}" style="display: inline-block; background: #007CB1; color: #0D1B2A; padding: 12px 30px; text-decoration: none; border-radius: 6px; font-weight: 600;">
        View Receipt
      </a>
    </div>
  `;
...
View your receipt: ${APP_URL}/q/${share_link}
  ...
  return {
    subject: `Payment Confirmed - ${aircraftDisplay} Detail with ${companyName}`,
...
${detailer?.email ? `Contact: ${detailer.email}` : ''}${detailer?.phone ? ` | ${detailer.phone}` : ''}
  ...
  return {
    subject: isNewQuote
      ? `Recurring Service Ready - ${aircraftDisplay} from ${companyName}`
      : `Service Reminder - ${aircraftDisplay} on ${dateStr}`,
...
${detailer?.email ? `Contact: ${detailer.email}` : ''}${detailer?.phone ? ` | ${detailer.phone}` : ''}
  ...
  return {
    subject: `Service Confirmed - ${aircraftDisplay} on ${dateStr}`,
...
${detailer?.email ? `Contact: ${detailer.email}` : ''}${detailer?.phone ? ` | ${detailer.phone}` : ''}
  ...
  return {
    subject: `Service Tomorrow - ${aircraftDisplay} with ${companyName}`,
...
export function jobCompletedTemplate({ quote, detailer, completedAt }) {
  ...
  const content = `
    ...
    <div style="background: #f8f6f0; border: 2px solid #007CB1; border-radius: 8px; padding: 20px; margin: 20px 0; text-align: center;">
      ...
      <p style="margin: 0; color: #007CB1; font-size: 20px; font-weight: 700;">${aircraftDisplay}</p>
...
${detailer?.email ? `Contact: ${detailer.email}` : ''}${detailer?.phone ? ` | ${detailer.phone}` : ''}
  ...
  return {
    subject: `Service Complete - ${aircraftDisplay} by ${companyName}`,
...
export function followUpTemplate({ quote, detailer }) {
  ...
  const content = `
    ...
    <div style="text-align: center; margin: 30px 0;">
      <a href="${APP_URL}/q/${share_link}" style="display: inline-block; background: linear-gradient(135deg, #007CB1 0%, #a88b3a 100%); color: #0D1B2A; padding: 16px 40px; text-decoration: none; border-radius: 8px; font-weight: 600; font-size: 16px;">
        View Quote &amp; Pay
      </a>
    </div>
...
Questions? Reply to this email or contact ${companyName} directly.
  ...
  return {
    subject: `Your ${aircraftDisplay} quote from ${companyName}`,
...
export function welcomeTemplate({ detailer }) {
  ...
  const content = `
    ...
    <p style="font-size: 16px; margin-bottom: 20px;">
      Welcome to <strong>Shiny Jets CRM</strong>! You're all set to start creating professional aircraft detailing quotes in minutes.
    </p>
    ...
    <div style="background: #f8fafc; border-radius: 8px; padding: 20px; margin: 20px 0;">
      ...
      <p style="margin: 16px 0 0 0; font-size: 14px; color: #6b7280;">
        Need unlimited quotes? <a href="${APP_URL}/settings" style="color: #007CB1; font-weight: 600;">Upgrade to Pro</a> for $79/mo.
      </p>
    </div>

    <div style="text-align: center; margin: 30px 0;">
      <a href="${APP_URL}/dashboard" style="display: inline-block; background: linear-gradient(135deg, #007CB1 0%, #a88b3a 100%); color: #0D1B2A; padding: 16px 40px; text-decoration: none; border-radius: 8px; font-weight: 600; font-size: 16px;">
        Go to Dashboard
      </a>
    </div>
...
Welcome to Shiny Jets CRM! You're all set to start creating professional aircraft detailing quotes.
...
Questions? Reply to this email and we'll help you get set up.
  ...
  return {
    ...
    html: emailWrapper(content, { headerBg: BRAND_COLOR, headerText: 'Welcome to Shiny Jets CRM!' }),
    text,
  };
}
...
export function feedbackRequestTemplate({ quote, detailer }) {
  ...
  const content = `
    ...
    <div style="text-align: center; margin: 24px 0;">
      <a href="${reviewBaseUrl}" style="display: inline-block; background: linear-gradient(135deg, #007CB1 0%, #a88b3a 100%); color: #0D1B2A; padding: 14px 36px; text-decoration: none; border-radius: 8px; font-weight: 600; font-size: 15px;">
        Leave a Review
      </a>
    </div>
...
export function lowStockAlertTemplate({ products, detailer }) {
  ...
  const content = `
    ...
    <div style="text-align: center; margin: 30px 0;">
      <a href="${APP_URL}/products" style="display: inline-block; background: #007CB1; color: #0D1B2A; padding: 14px 36px; text-decoration: none; border-radius: 6px; font-weight: 600; font-size: 16px;">
        View Inventory
      </a>
    </div>
  `;
...
export function quoteExpiringTemplate({ quote, detailer }) {
  ...
  const content = `
    ...
    <div style="text-align: center; margin: 30px 0;">
      <a href="${quoteLink}" style="display: inline-block; background: linear-gradient(135deg, #007CB1 0%, #a88b3a 100%); color: #0D1B2A; padding: 14px 36px; text-decoration: none; border-radius: 6px; font-weight: 600; font-size: 16px;">
  ...
  return {
    subject: `Your ${aircraftDisplay} quote expires tomorrow`,
...
export function quoteExpiredDetailerTemplate({ quote, detailer }) {
  ...
  const content = `
    ...
    <div style="text-align: center; margin: 30px 0;">
      <a href="${APP_URL}/quotes" style="display: inline-block; background: #007CB1; color: #0D1B2A; padding: 12px 30px; text-decoration: none; border-radius: 6px; font-weight: 600;">
        View in Dashboard
      </a>
    </div>
  `;
...
export function invoiceTemplate({ invoice }) {
  ...
  const invLabel = invoice.invoice_number || (invoice.id ? `#${String(invoice.id).slice(0, 8).toUpperCase()}` : '');
  const fromLogo = (invoice.detailer_logo_light_url || invoice.detailer_logo_url) || '';
  const fromName = invoice.detailer_company || invoice.detailer_name || '';

  let itemsHtml = '';
  if (items.length > 0) {
    const rows = items.map(item => `
      <tr>
        <td style="padding: 8px 0; border-bottom: 1px solid #f3f4f6;">${item.name || item.description || item.service || 'Service'}</td>
  ...
  // Platform fee is included in total but not shown to customer
  const feeHtml = '';
  ...
  const content = `
    <div style="margin-bottom: 24px;">
      <div style="display: flex; justify-content: space-between; align-items: flex-start;">
        <div>
          <h2 style="margin: 0 0 4px 0; color: #1f2937; font-size: 20px;">Invoice ${invLabel}</h2>
    ...
    <div style="background: #f9fafb; padding: 16px; border-radius: 8px; margin-bottom: 16px;">
      <div style="margin-bottom: 12px;">
        ...
        ${fromLogo ? `<img src="${fromLogo}" alt="" style="height:36px;max-width:180px;object-fit:contain;display:block;margin:4px 0 6px" />` : ''}
        ${fromName ? `<p style="margin: 2px 0 0 0; font-weight: 600; color: #1f2937;">${fromName}</p>` : ''}
        ${invoice.detailer_name && invoice.detailer_name !== fromName ? `<p style="margin: 0; color: #6b7280; font-size: 14px;">${invoice.detailer_name}</p>` : ''}
        ...
        ${invoice.detailer_home_airport ? `<p style="margin: 0; color: #6b7280; font-size: 14px;">${invoice.detailer_home_airport}</p>` : ''}
      </div>
      <div>
    ...
    <div style="border-top: 2px solid #1e3a5f; padding-top: 12px; margin-top: 12px;">
      ...
      ${(() => {
        ...
        const discRow = (discAmt > 0 || parseFloat(invoice.discount_value) > 0)
          ? `<div style="display: flex; justify-content: space-between; align-items: center; font-size: 14px; color: #dc2626; margin-bottom: 8px;"><span>Discount (${invoice.discount_type === 'flat' ? `${formatCurrency(invoice.discount_value)} off` : `${parseFloat(invoice.discount_value) || 0}% off`}${invoice.discount_reason ? ` &mdash; ${invoice.discount_reason}` : ''})</span><span>-${formatCurrency(discAmt)}</span></div>`
          : '';
        return `<div style="display: flex; justify-content: space-between; align-items: center; font-size: 14px; color: #6b7280; margin-bottom: 4px;"><span>Subtotal</span><span style="color:#1f2937;font-weight:600">${formatCurrency(sub)}</span></div>${discRow}`;
      })()}
      <div style="display: flex; justify-content: space-between; align-items: center;">
  ...
  const text = `Invoice ${invLabel}\nFrom: ${invoice.detailer_company || invoice.detailer_name}\nTo: ${invoice.customer_name}\nTotal: ${formatCurrency(invoice.total)}\nStatus: ${statusLabel}`;

  return {
    subject: `Invoice ${invLabel} from ${invoice.detailer_company || invoice.detailer_name || 'your detailing professional'}`,
    html: emailWrapper(content, {
      headerBg: BRAND_COLOR,
      headerText: `Invoice ${invLabel}`,
...
export function followUpReminderTemplate({ clientName, aircraft, quoteUrl, expiresIn, detailerName, detailerPhone }) {
  ...
  const content = `
    ...
    <div style="text-align: center; margin: 30px 0;">
      <a href="${quoteUrl}" style="display: inline-block; background: #007CB1; color: #0D1B2A; padding: 14px 36px; text-decoration: none; border-radius: 6px; font-weight: 600; font-size: 16px;">
        View Your Quote
      </a>
    </div>
  ...
  return {
    subject: `Your ${aircraftDisplay} quote from ${company} expires in ${expiresIn}`,
...
export function expiryDiscountTemplate({ clientName, aircraft, quoteUrl, discountPercent, originalPrice, discountedPrice, detailerName, detailerPhone, currency }) {
  ...
  const content = `
    ...
    <p style="font-size: 15px; color: #374151; margin-bottom: 20px;">
      Hi ${name},<br><br>
      Your quote for <strong>${aircraftDisplay}</strong> expires in <strong>2 days</strong>. Book now and save <strong style="color: #007CB1;">${discountPercent}%</strong>!
    </p>

    <div style="background: #f8f6f0; border: 1px solid #007CB1; border-radius: 8px; padding: 20px; margin-bottom: 20px; text-align: center;">
      ...
      <p style="margin: 4px 0 0 0; font-size: 28px; font-weight: 700; color: #007CB1;">${discounted}</p>
      <p style="margin: 8px 0 0 0; font-size: 14px; color: #8B7332; font-weight: 600;">Save ${discountPercent}%</p>
    </div>

    <div style="text-align: center; margin: 30px 0;">
      <a href="${quoteUrl}" style="display: inline-block; background: linear-gradient(135deg, #007CB1 0%, #a88b3a 100%); color: #0D1B2A; padding: 14px 36px; text-decoration: none; border-radius: 6px; font-weight: 600; font-size: 16px;">
        Claim Your Discount
      </a>
    </div>
    ...
    <div style="background: linear-gradient(135deg, #1e3a5f 0%, #0f172a 100%); border-radius: 8px; padding: 24px; margin: 24px 0; color: #fff;">
      ...
      <div style="display: flex; justify-content: space-between; margin-bottom: 16px;">
        <div style="text-align: center; flex: 1;">
          <p style="margin: 0; font-size: 28px; font-weight: 700; color: #007CB1;">${fmtCurrency(stats.totalRevenue)}</p>
      ...
      <table style="width: 100%; border-collapse: collapse;">
        ...
        <tr>
          ...
          <td style="padding: 8px 0; border-top: 1px solid rgba(255,255,255,0.1); font-size: 13px; color: #007CB1; text-align: right; font-weight: 600;">${fmtCurrency(stats.avgJobValue)}</td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td style="padding: 12px 0; border-top: 2px solid rgba(201,168,76,0.3); font-size: 15px; color: #007CB1; text-align: right; font-weight: 700;">${fmtCurrency(stats.netRevenue)}</td>
        </tr>
      </table>
    </div>

    <div style="text-align: center; margin: 28px 0;">
      <a href="${APP_URL}/reports" style="display: inline-block; background: #007CB1; color: #ffffff; text-decoration: none; padding: 12px 32px; border-radius: 4px; font-weight: 600; font-size: 14px; letter-spacing: 0.5px;">
        View Full Reports
      </a>
    </div>

    <p style="font-size: 12px; color: #999; text-align: center;">
      You can manage this email in <a href="${APP_URL}/settings" style="color: #007CB1;">Settings</a>.
    </p>
  `;
...
export function betaInviteTemplate({ email, plan, durationDays, note, token }) {
  ...
  const content = `
    <p style="font-size: 16px; color: #333; margin-bottom: 20px;">
      You've been personally invited to try <strong>Shiny Jets CRM</strong> — the premium quoting and client management platform built for aircraft detailing professionals.
    </p>

    <div style="background: #f8f6f0; border-left: 3px solid #007CB1; padding: 16px 20px; margin: 24px 0; border-radius: 0 8px 8px 0;">
      ...
      <p style="margin: 0 0 4px 0; font-size: 15px; color: #333;"><strong>Duration:</strong> ${durationDays}-day trial included</p>
      <p style="margin: 0; font-size: 15px; color: #333;"><strong>Cost:</strong> No charge during trial</p>
    </div>

    ${note ? `<p style="font-size: 14px; color: #555; font-style: italic; margin: 16px 0;">"${note}"</p>` : ''}

    <div style="text-align: center; margin: 32px 0;">
      <a href="${signupUrl}" style="display: inline-block; background: #007CB1; color: #ffffff; text-decoration: none; padding: 14px 36px; border-radius: 4px; font-weight: 600; font-size: 15px; letter-spacing: 0.5px;">
        Accept Invitation
      </a>
    </div>
  ...
  const text = `You're invited to Shiny Jets CRM.\n\nPlan: ${planLabel}\nDuration: ${durationDays}-day trial\n\n${note ? `Note: ${note}\n\n` : ''}Accept your invitation: ${signupUrl}\n\nThis invite is for ${email} only.`;

  return {
    subject: `Your invitation to Shiny Jets CRM`,
    html: emailWrapper(content, { headerBg: '#007CB1', headerText: 'Beta Invitation' }),
    text,
  };
}
...
export function customerIntroTemplate({ customerName, detailerCompany, detailerName }) {
  ...
  const content = `
    ...
    <p style="font-size: 15px; color: #333; line-height: 1.6; margin-bottom: 20px;">
      <strong>${detailerCompany}</strong> has invited you to <strong>Shiny Jets CRM</strong>, their aircraft detailing management platform.
    </p>

    <p style="font-size: 15px; color: #333; line-height: 1.6; margin-bottom: 20px;">
      Going forward, you'll receive professional quotes, invoices, and service updates through Shiny Jets CRM. No action is needed on your end — when ${detailerName || detailerCompany} sends you a quote, you'll receive it directly to this email.
    </p>
  ...
  const text = `${greeting}\n\n${detailerCompany} has invited you to Shiny Jets CRM, their aircraft detailing management platform.\n\nGoing forward, you'll receive professional quotes, invoices, and service updates through Shiny Jets CRM.\n\nLearn more: ${APP_URL}`;

  return {
    subject: `${detailerCompany} is now on Shiny Jets CRM`,
    html: emailWrapper(content, { headerBg: BRAND_COLOR, headerText: 'Welcome to Shiny Jets CRM', companyName: detailerCompany }),
    text,
  };
}
...
export function followupNotViewedTemplate({ quote, detailer }) {
  ...
  const content = `
    ...
    <div style="text-align: center; margin: 30px 0;">
      <a href="${quoteLink}" style="display: inline-block; background: linear-gradient(135deg, #007CB1 0%, #a88b3a 100%); color: #0D1B2A; padding: 16px 40px; text-decoration: none; border-radius: 8px; font-weight: 600; font-size: 16px;">
        View Your Quote
      </a>
    </div>
...
export function referralRewardReferrerTemplate({ referrer, referredName, pointsEarned }) {
  ...
  const content = `
    ...
    <p style="font-size: 16px; margin-bottom: 20px;">
      Great news! <strong>${referredName}</strong> just completed their first paid quote on Shiny Jets CRM. As a thank you for the referral, here's what you've earned:
    </p>
    ...
    <div style="text-align: center; margin: 30px 0;">
      <a href="${dashboardUrl}" style="display: inline-block; background: linear-gradient(135deg, #007CB1 0%, #a88b3a 100%); color: #0D1B2A; padding: 14px 36px; text-decoration: none; border-radius: 8px; font-weight: 600; font-size: 15px;">
        View Your Referrals
      </a>
    </div>
  `;

  const text = `Hi ${name},\n\nGreat news! ${referredName} just completed their first paid quote on Shiny Jets CRM.\n\nYou've earned:\n- ${pointsEarned.toLocaleString()} bonus points\n- 30 days of free Pro access\n\nKeep sharing your referral link to earn more rewards.\n\nView your referrals: ${dashboardUrl}`;
...
export function referralRewardReferredTemplate({ referred, pointsEarned }) {
  ...
  const content = `
    ...
    <p style="font-size: 16px; margin-bottom: 20px;">
      Congratulations on completing your first paid quote on Shiny Jets CRM! As part of our referral program, you've earned a welcome bonus:
    </p>
...
export function followupExpiredRecoveryTemplate({ quote, detailer }) {
  ...
  const content = `
    ...
    <div style="text-align: center; margin: 30px 0;">
      <a href="${requestLink}" style="display: inline-block; background: linear-gradient(135deg, #007CB1 0%, #a88b3a 100%); color: #0D1B2A; padding: 16px 40px; text-decoration: none; border-radius: 8px; font-weight: 600; font-size: 16px;">
        Request New Quote
      </a>
    </div>
...
export function invoiceReminderTemplate({ invoice }) {
  ...
  const content = `
    <div style="display:none;font-size:1px;color:#f3f4f6;line-height:1px;max-height:0px;max-width:0px;opacity:0;overflow:hidden;">
      Payment reminder for Invoice ${invLabel} from ${companyName}
    </div>
    ...
    <div style="background: ${isOverdue ? '#fef2f2' : '#fffbeb'}; border: 1px solid ${isOverdue ? '#fecaca' : '#fde68a'}; border-radius: 12px; padding: 20px; margin: 24px 0; text-align: center;">
      <p style="margin: 0 0 4px 0; color: #6b7280; font-size: 13px;">Invoice ${invLabel}</p>
  ...
  const text = `Hi${invoice.customer_name ? ` ${invoice.customer_name}` : ''},\n\nThis is a reminder that payment of ${formatCurrency(balanceDue)} is ${isOverdue ? 'overdue' : 'due'} for Invoice ${invLabel} from ${companyName}.${dueDateStr ? `\nDue date: ${dueDateStr}` : ''}\n\nPlease arrange payment at your earliest convenience.\n\nQuestions? Contact ${invoice.detailer_email || companyName}.`;

  return {
    subject: `${isOverdue ? 'Overdue: ' : ''}Payment Reminder — Invoice ${invLabel}`,
    html: emailWrapper(content, {
...
export function bookingConfirmedTemplate({ quote, detailer }) {
  ...
  const content = `
    ...
    <div style="background: #f8f6f0; border: 2px solid #007CB1; border-radius: 8px; padding: 20px; margin: 20px 0; text-align: center;">
      ...
      <p style="margin: 0; color: #007CB1; font-size: 28px; font-weight: 700;">${aircraftDisplay}</p>
    </div>
    ...
    <div style="text-align: center; margin: 30px 0;">
      <a href="${APP_URL}/q/${quote.share_link}" style="display: inline-block; background: #007CB1; color: #0D1B2A; padding: 12px 30px; text-decoration: none; border-radius: 6px; font-weight: 600;">
        View Quote
      </a>
    </div>
  `;
...
export function airportLimitWarningTemplate({
  company,
  plan,
  primaryCurrent,
  primaryLimit,
  ...
  const deadline = new Date(deadlineIso).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
  const planLabel = plan ? plan.charAt(0).toUpperCase() + plan.slice(1) : 'current';
  const airportsUrl = `${APP_URL}/settings/locations`;
  const upgradeUrl = 'https://pricing.shinyjets.com';

  // Build the per-tier overage list. Only mention a tier when it's actually
  // over so detailers who blow past one tier and not the other get a clean
  // single-line ask.
  const tierLines = [];
  const tierLinesText = [];
  if (primaryOver > 0) {
    tierLines.push(
      `<li><strong>Primary airports:</strong> ${primaryCurrent} active, your ${planLabel} plan includes ${primaryLimit} (${primaryOver} over).</li>`,
    );
    tierLinesText.push(`  - Primary airports: ${primaryCurrent} active, ${planLabel} plan includes ${primaryLimit} (${primaryOver} over).`);
  }
  if (secondaryOver > 0) {
    tierLines.push(
      `<li><strong>Secondary airports:</strong> ${secondaryCurrent} active, your ${planLabel} plan includes ${secondaryLimit} (${secondaryOver} over).</li>`,
    );
    tierLinesText.push(`  - Secondary airports: ${secondaryCurrent} active, ${planLabel} plan includes ${secondaryLimit} (${secondaryOver} over).`);
  }

  const bothOver = primaryOver > 0 && secondaryOver > 0;
  const headline = bothOver
    ? 'both your primary and secondary airport tiers are over their plan limit'
    : primaryOver > 0
      ? 'your primary airport tier is over its plan limit'
      : 'your secondary airport tier is over its plan limit';
  const headlineText = bothOver
    ? 'both your primary and secondary airport tiers are over their plan limit'
    : primaryOver > 0
      ? 'your primary airport tier is over its plan limit'
      : 'your secondary airport tier is over its plan limit';
  ...
  const content = `
    <p style="font-size:15px;color:#1a1a1a;line-height:1.6;margin:0 0 16px 0;">Hi ${company},</p>
    <p style="font-size:15px;color:#1a1a1a;line-height:1.6;margin:0 0 16px 0;">Heads up — ${headline}. Here's the breakdown:</p>
    <ul style="font-size:15px;color:#1a1a1a;line-height:1.8;margin:0 0 20px 0;padding-left:20px;">
      ${tierLines.join('\n      ')}
    </ul>
    <p style="font-size:14px;color:#555;line-height:1.6;margin:0 0 20px 0;font-style:italic;">Primary airports are home bases — they rank above secondaries in the public directory.</p>
    <p style="font-size:15px;color:#1a1a1a;line-height:1.6;margin:0 0 8px 0;font-weight:600;">You have until ${deadline} to take action:</p>
    <ul style="font-size:15px;color:#1a1a1a;line-height:1.8;margin:0 0 20px 0;padding-left:20px;">
      <li>Trim each over-limit tier down to your plan allowance, or</li>
      <li>Upgrade your plan to keep them all active.</li>
    </ul>
    <p style="font-size:15px;color:#1a1a1a;line-height:1.6;margin:0 0 24px 0;">If no action is taken by ${deadline}, we'll automatically deactivate your oldest excess airports in each over-limit tier so your account stays inside the ${planLabel} plan. Your aircraft, customers, and history all stay intact — only the extra airport rows go inactive, and they can be reactivated any time you upgrade or remove other airports.</p>
    <table cellpadding="0" cellspacing="0" border="0" style="margin:0 0 24px 0;">
      <tr>
        <td style="padding-right:8px;">
          <a href="${airportsUrl}" style="display:inline-block;padding:12px 22px;background:#0D1B2A;color:#ffffff;text-decoration:none;border-radius:6px;font-size:14px;font-weight:600;">Manage airports</a>
        </td>
        <td>
          <a href="${upgradeUrl}" style="display:inline-block;padding:12px 22px;background:#007CB1;color:#ffffff;text-decoration:none;border-radius:6px;font-size:14px;font-weight:600;">Upgrade plan</a>
        </td>
      </tr>
    </table>
    <p style="font-size:14px;color:#555;line-height:1.6;margin:0;">Questions? Just reply to this email and we'll get back to you.</p>
    <p style="font-size:14px;color:#555;line-height:1.6;margin:16px 0 0 0;">— The Shiny Jets team</p>
  `;

  const text = `Hi ${company},

Heads up — ${headlineText}. Here's the breakdown:

${tierLinesText.join('\n')}

Primary airports are home bases — they rank above secondaries in the public directory.

You have until ${deadline} to either trim each over-limit tier down to your plan allowance or upgrade your plan to keep them all active.

If no action is taken by ${deadline}, the oldest excess airports in each over-limit tier will be deactivated automatically. Your aircraft, customers, and history stay intact — only the extra airport rows go inactive, and they can be reactivated any time you upgrade or remove other airports.

Manage airports: ${airportsUrl}
...
— The Shiny Jets team`;
  ...
  return {
    subject: `Action needed: your Shiny Jets CRM airports exceed your ${planLabel} plan limit`,
    html: emailWrapper(content, { headerBg: WARNING_COLOR, headerText: 'Airport limit notice' }),
    text,
  };
}

export function airportLimitEnforcedTemplate({ company, plan, primaryLimit, secondaryLimit, deactivated }) {
  const planLabel = plan ? plan.charAt(0).toUpperCase() + plan.slice(1) : 'current';
  const upgradeUrl = 'https://pricing.shinyjets.com';
  const airportsUrl = `${APP_URL}/settings/locations`;
  const rows = deactivated || [];
  const tierBadgeHtml = (tier) => {
    const t = String(tier || 'secondary').toLowerCase();
    const isPrimary = t === 'primary';
    const bg = isPrimary ? '#0D1B2A' : '#6b7280';
    const label = isPrimary ? 'Primary' : 'Secondary';
    return `<span style="display:inline-block;padding:2px 8px;margin-left:6px;background:${bg};color:#ffffff;border-radius:999px;font-size:11px;font-weight:600;vertical-align:middle;">${label}</span>`;
  };
  const tierBadgeText = (tier) => {
    const t = String(tier || 'secondary').toLowerCase();
    return t === 'primary' ? '[primary]' : '[secondary]';
  };
  const list = rows.map(d => `<li>${d.name}${d.icao ? ` (${d.icao})` : ''}${tierBadgeHtml(d.tier)}</li>`).join('');
  const listText = rows.map(d => `  - ${d.name}${d.icao ? ` (${d.icao})` : ''} ${tierBadgeText(d.tier)}`).join('\n');

  const content = `
    <p style="font-size:15px;color:#1a1a1a;line-height:1.6;margin:0 0 16px 0;">Hi ${company},</p>
    <p style="font-size:15px;color:#1a1a1a;line-height:1.6;margin:0 0 16px 0;">Your 30-day notice period ended, so we deactivated <strong>${rows.length} airport${rows.length === 1 ? '' : 's'}</strong> to bring your account inside the <strong>${planLabel} plan</strong> limit (${primaryLimit} primary / ${secondaryLimit} secondary).</p>
    <p style="font-size:15px;color:#1a1a1a;line-height:1.6;margin:0 0 8px 0;font-weight:600;">Deactivated airports:</p>
    <ul style="font-size:15px;color:#1a1a1a;line-height:1.8;margin:0 0 20px 0;padding-left:20px;">
      ${list}
    </ul>
    <p style="font-size:15px;color:#1a1a1a;line-height:1.6;margin:0 0 16px 0;">Nothing was deleted — all your aircraft, customers, and history are still here. The airports themselves are just inactive. You can restore them in two ways:</p>
    <ul style="font-size:15px;color:#1a1a1a;line-height:1.8;margin:0 0 20px 0;padding-left:20px;">
      <li>Upgrade your plan and reactivate them from the airports page.</li>
      <li>Remove other airports first to free up room, then reactivate the ones you want.</li>
    </ul>
    <table cellpadding="0" cellspacing="0" border="0" style="margin:0 0 24px 0;">
      <tr>
        <td style="padding-right:8px;">
    ...
    <p style="font-size:14px;color:#555;line-height:1.6;margin:0;">Questions or need a hand? Just reply to this email.</p>
    <p style="font-size:14px;color:#555;line-height:1.6;margin:16px 0 0 0;">— The Shiny Jets team</p>
  `;

  const text = `Hi ${company},

Your 30-day notice period ended, so we deactivated ${rows.length} airport${rows.length === 1 ? '' : 's'} to bring your account inside the ${planLabel} plan limit (${primaryLimit} primary / ${secondaryLimit} secondary).

Deactivated airports:
${listText}

Nothing was deleted — all your aircraft, customers, and history are still here. The airports themselves are just inactive. You can restore them by upgrading your plan and reactivating from the airports page, or by removing other airports first to free up room.
...
— The Shiny Jets team`;
  ...
  return {
    subject: `${rows.length} of your airports were deactivated — Shiny Jets CRM`,
    html: emailWrapper(content, { headerBg: '#b91c1c', headerText: 'Airports deactivated' }),
    text,
  };
}

// Comp invite welcome — sent when an admin stages a comp_invites row to let
// the recipient know a free plan is waiting for them at signup. The plan
// auto-applies on first signup via lib/comp-invites.redeemCompInviteIfAny.
// IMPORTANT: do not personalize with a first name we don't have; addressing
// just "Hey," keeps the copy honest and matches Brett's voice.
export function compInviteTemplate({ email }) {
  const signupUrl = 'https://crm.shinyjets.com/signup';
  const safeEmail = String(email || '').trim();

  const content = `
    <p style="font-size:16px;line-height:1.6;margin:0 0 16px;color:#1a1a1a;">Hey,</p>

    <p style="font-size:15px;line-height:1.6;margin:0 0 16px;color:#1a1a1a;">
      Setting you up with a free year of the Shiny Jets CRM, <strong>Enterprise</strong> plan &mdash; comped because you took the course. That includes <strong>Detailing AI</strong> inside the CRM.
    </p>

    <div style="background:#f0f7fb;border:1px solid #cfe4f0;border-radius:8px;padding:16px 18px;margin:20px 0;">
      <p style="font-size:12px;color:#666;text-transform:uppercase;letter-spacing:0.05em;margin:0 0 6px;">Use this exact email at signup</p>
      <p style="margin:0;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:15px;color:#1a1a1a;font-weight:600;">${safeEmail}</p>
      <p style="font-size:12px;color:#555;margin:10px 0 0;">That's how the Enterprise comp applies automatically. Sign in with Google works too &mdash; just make sure the Google account uses this address.</p>
    </div>

    <div style="text-align:center;margin:24px 0;">
      <a href="${signupUrl}" style="display:inline-block;padding:14px 28px;background:#007CB1;color:#fff;text-decoration:none;border-radius:8px;font-weight:600;font-size:15px;">Sign up &mdash; Enterprise applies automatically</a>
    </div>

    <p style="font-size:14px;line-height:1.6;margin:20px 0 8px;color:#1a1a1a;font-weight:600;">What you get on Enterprise:</p>
    <ul style="font-size:14px;line-height:1.7;margin:0 0 18px;padding-left:20px;color:#1a1a1a;">
      <li>Detailing AI in CRM (Insights → Detailing AI)</li>
      <li>Full CRM, customer database, invoicing, quoting, white-label</li>
      <li>Job dispatch + crew management · 0% platform fee</li>
      <li>Rewards: earn points → redeem for free products, a free CRM month, and Fly Shiny gear at <a href="https://flyshiny.com" style="color:#007CB1;">flyshiny.com</a></li>
    </ul>

    <p style="font-size:14px;line-height:1.6;margin:18px 0 4px;color:#1a1a1a;">Good for one year. Holler if anything breaks or you need a hand.</p>
    <p style="font-size:14px;line-height:1.6;margin:0;color:#1a1a1a;">&mdash; Brett<br/>sales@shinyjets.com · shinyjets.com</p>
  `;

  const text = `Hey,

Setting you up with a free year of the Shiny Jets CRM, Enterprise plan — comped because you took the course. Detailing AI included.

Sign up here: ${signupUrl}

IMPORTANT: use this exact email address when you sign up so the comp applies automatically: ${safeEmail}

You can use either Sign in with Google OR create a password — either works.

What you get on Enterprise:
- Detailing AI in CRM
- Full CRM, invoicing, quoting, white-label
- Job dispatch + crew · 0% platform fee
- Rewards: redeem points for free products, a free CRM month, and Fly Shiny gear at https://flyshiny.com

Good for one year. Holler if anything breaks or you need a hand.
...
shinyjets.com`;
  ...
  return {
    subject: 'Your free year of Shiny Jets CRM Enterprise is ready',
    html: emailWrapper(content, { headerBg: '#0D1B2A', headerText: 'Your free Enterprise year' }),
    text,
  };
}
