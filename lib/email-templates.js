...
export function invoiceTemplate({ invoice }) {
  ...
  const content = `
    ...
    <div style="border-top: 2px solid #1e3a5f; padding-top: 12px; margin-top: 12px;">
      ...
      ${(parseFloat(invoice.discount_amount) > 0 || parseFloat(invoice.discount_value) > 0) ? `
        <div style="display: flex; justify-content: space-between; align-items: center; font-size: 14px; color: #6b7280; margin-bottom: 4px;">
          <span>Subtotal</span>
          <span>${formatCurrency(parseFloat(invoice.subtotal) || (parseFloat(invoice.total) + parseFloat(invoice.discount_amount) || 0))}</span>
        </div>
        <div style="display: flex; justify-content: space-between; align-items: center; font-size: 14px; color: #dc2626; margin-bottom: 8px;">
          <span>Discount (${invoice.discount_type === 'flat' ? `${formatCurrency(invoice.discount_value)} off` : `${parseFloat(invoice.discount_value) || 0}% off`}${invoice.discount_reason ? ` &mdash; ${invoice.discount_reason}` : ''})</span>
  ...
  return {
    subject: `Invoice ${invoice.invoice_number} from ${invoice.detailer_company || invoice.detailer_name || 'your detailing professional'}`,
