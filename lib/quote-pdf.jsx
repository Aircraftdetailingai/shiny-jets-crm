...
// (app/api/quotes/[id]/send/route.js) both call generateQuotePdf() so the
// attached PDF is byte-identical to the one customers view — no HTTP round-trip.
...
// Filename used both for the Content-Disposition header and the email attachment.
export function quotePdfFilename(quote) {
  return `Quote-${(quote.aircraft_model || quote.aircraft_type || 'Aircraft').replace(/[^a-zA-Z0-9]/g, '-')}-${quote.id.slice(0, 8).toUpperCase()}.pdf`;
}

// Generate the quote PDF from an already-fetched quote row. Fetches the
// detailer (with the columns the PDF needs) and derives line items, services,
// addon fees, and package info — the same derivation the customer route uses.
// Returns { buffer, filename }.
export async function generateQuotePdf(quote, { supabase } = {}) {
  ...
  return { buffer, filename: quotePdfFilename(quote) };
}
