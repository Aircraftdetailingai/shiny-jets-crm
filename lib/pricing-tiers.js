// Pricing tiers configuration for Shiny Jets CRM freemium model
...
export const TIERS = {
  ...
  pro: {
    ...
    stripePriceId: process.env.STRIPE_PRICE_PRO || 'price_1TGnugCcQQZWc7n0Ge8tsTQC',
  ...
  business: {
    ...
    stripePriceId: process.env.STRIPE_PRICE_BUSINESS || 'price_1TGnvHCcQQZWc7n0uEDAqRvb',
  ...
  enterprise: {
    ...
    features: [
      ...
      'FlightAware flight hour tracking',
      'Fuel receipt upload portal',
      'Pre/post ceramic fuel burn comparison',
      'Automated ROI reporting',
      'Priority support',
      '0% platform fee',
    ],
    stripePriceId: process.env.STRIPE_PRICE_ENTERPRISE || 'price_1TGnvhCcQQZWc7n0Cibeq0tN',
...
// Reads detailer.platform_fee_percent (DB column, supports per-detailer
// overrides like comps) and falls back to the plan-based table when the
// column is null/undefined. This is the canonical source for Stripe
// application_fee_amount across all charge sites.
export function resolveFeeRate(detailer) {
  ...
  return PLATFORM_FEES[plan] ?? PLATFORM_FEES.free;
}
