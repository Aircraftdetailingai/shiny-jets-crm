...
export async function PUT(request, { params }) {
  try {
    ...
    const { name, description, hourly_rate, hours_field, product_cost_per_hour, product_notes, default_hours, category, minimum_price, sop_url, sop_summary, sop_file_path, sop_file_name } = body;
    ...
    // Level 1 service SOP — Stage 1 of the SOP feature. All four fields
    // are optional; empty string is treated as "clear" (null in DB) so
    // the owner can remove without juggling null/undefined in the UI.
    // sop_file_path / sop_file_name pair go together — the path lives in
    // the private sop-documents bucket; the name preserves the original
    // upload filename for display.
