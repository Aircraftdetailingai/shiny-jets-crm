...
function IntegrationsContent() {
  ...
  useEffect(() => {
    ...
    if (gcalParam === 'success') {
      ...
      // Refresh status before stripping query so a slow upsert still paints Connected
      (async () => {
          ...
          <div className="border border-v-border p-5 flex flex-col">
            ...
            {!gcalLoading && !gcalConnected && !gcalNeedsReconnect && (
              <div className="flex-1 space-y-3">
                {/* Google OAuth button */}
                {crmEmail && (
                  <p className="text-[10px] text-v-text-secondary mb-1">
                    Connecting for CRM account <span className="text-v-text-primary">{crmEmail}</span>
                  </p>
                )}
                <button onClick={handleConnectGCal} disabled={gcalConnecting}
